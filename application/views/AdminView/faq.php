...
<div class="content-wrapper">
	<div class="container-fluid">
		<div class="content-header mr-5 ml-5">
			...
			<br>
			<button type="button" class="text-left btn btn-success mt-2 form-control" data-toggle="modal" data-target="#exampleModal"><i class="fa fa-plus mr-2"></i> Tambah FAQ</button>
			<br><br>
			<?= $this->session->flashdata('pesan') ?>
		</div>
		<div class="content-content bg-white pt-4 pr-3 pl-3 pb-3">
			<h5 class="text-center"><strong>FAQ</strong></h5>
			<div class="section-3">
					...
					<table class="table table-striped table-bordered">
						...
						<tr class="">
							...
							<?php if( $pertanyaan->jawaban == '' ) : ?>
								<td>
									<form method="post" action="<?= base_url('Administrator/FAQ/EditJawaban/'.$pertanyaan->id_faq) ?>">
										<textarea class="ckeditor" id="ckeditor" name="jawaban"></textarea>
										<input type="submit" name="" class="btn btn-secondary mt-1" value="Jawab">
									</form>
								</td>
							<?php else :?>
								<td id="jawaban_<?= $pertanyaan->id_faq ?>"><?= $pertanyaan->jawaban ?></td>
								<td id="editjawaban_<?= $pertanyaan->id_faq ?>" style="display: none;">
									<div>
										<form method="post" action="<?= base_url('Administrator/FAQ/EditJawaban/'.$pertanyaan->id_faq) ?>">
											<textarea class="ckeditor" id="ckeditor" name="jawaban"><?= $pertanyaan->jawaban ?></textarea>
											<input type="submit" name="" class="btn btn-secondary mt-1" value="Jawab">
											<a href="javascript:close('<?= $pertanyaan->id_faq ?>');" type="button" name="" class="btn btn-danger mt-1">Tutup</a>
										</form>
									</div>
								</td>
							<?php endif;?>
							...
							<td>
								<a href="javascript:toggle('<?= $pertanyaan->id_faq ?>');" id="edit_<?= $pertanyaan->id_faq ?>">
									<h3><i class="fa fa-edit"></i></h3>
								</a>
							</td>
							<td><a href="<?= base_url('Administrator/FAQ/HapusJawaban/'.$pertanyaan->id_faq) ?>"><h3><i class="text-danger fa fa-trash"></i></h3></a></td>
						</tr>
						<?php
							endforeach;
						?>
...
<div class="modal fade" style="z-index: 10000000" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  	<div class="modal-dialog" role="document">
    	<div class="modal-content">
    		<form action="<?= base_url('Administrator/FAQ/TambahFAQ') ?>" method="post">
	      		...
	      		<div class="modal-body">
        			...
        			<div class="form-group">
        				<label>Jawaban</label>
        				<input type="password" name="jawaban" class="form-control">
        			</div>
	      		</div>
