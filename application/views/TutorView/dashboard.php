			...
			<div class="p-3">
				<?php foreach( $pengajar as $tutor ) : ?>
					<div class="load-left text-center">
						<img src="<?= base_url('assets/mystyle/img/tutor/'.$tutor->gambar_guru) ?>" style="width: 100%;">
						<h2><strong><?= $tutor->nama_panggilan_tutor ?></strong></h2>
					</div>
					<div class="load-right p-3">
						...
						<div class="float-left col-md-5 mb-3">
							<div class="bg-warning statistik text-center">
								...
								<?php 
									$query = "SELECT * FROM tb_pesanan_les WHERE id_tutor = '".$tutor->id_tutor."'";
									$que = $this->db->query($query)->num_rows();
								?>
								<h1 class="float-right col-5" style="font-size: 300%;"><strong><?= $que ?></strong></h1>
								<div class="clearfix"></div>
								<h5 class="read-statistik bg-dark p-1"><a class="text-dark" href="<?= site_url('Tutor/Jadwal_Les') ?>" >Read More >> </a></h5>
							</div>
						</div>
						<div class="float-right col-md-5 mb-3">
							<div class="bg-success statistik text-center">
								<h5 class="title-statistik p-1 mb-3">Komisi</h5>
								<i class="fas fa-dollar-sign float-left col-5" style="font-size: 300%;"></i>
								<?php 
									$query = "SELECT * FROM tb_komisi_tutor WHERE id_tutor = '".$tutor->id_tutor."'";
									$que = $this->db->query($query)->num_rows();
								?>
								<h1 class="float-right col-5" style="font-size: 300%;"><strong><?= $que ?></strong></h1>
								<div class="clearfix"></div>
								<h5 class="read-statistik bg-dark p-1"><a class="text-dark" href="<?= site_url('Tutor/Komisi') ?>" >Read More >> </a></h5>
							</div>
						</div>
						<div class="clearfix"></div>
						<div class="float-left col-md-5 mb-3">
							<div class="bg-primary statistik text-center">
								...
								<?php 
									$query = "SELECT * FROM tb_testimoni_tutor WHERE id_tutor = '".$tutor->id_tutor."'";
									$que = $this->db->query($query)->num_rows();
								?>
								<h1 class="float-right col-5" style="font-size: 300%;"><strong><?= $que ?></strong></h1>
								<div class="clearfix"></div>
								<h5 class="read-statistik bg-dark p-1"><a class="text-dark" href="<?= site_url('Tutor/Beri_Testimoni') ?>" >Read More >> </a></h5>
							</div>
						</div>
						<div class="float-right col-md-5 mb-3">
							<div class="bg-danger statistik text-center">
								...
								<?php 
									$query = "SELECT * FROM tb_kotaksaran WHERE id = '".$tutor->id_tutor."'";
									$que = $this->db->query($query)->num_rows();
								?>
								<h1 class="float-right col-5" style="font-size: 300%;"><strong><?= $que ?></strong></h1>
								<div class="clearfix"></div>
								<h5 class="read-statistik bg-dark p-1"><a class="text-dark" href="<?= site_url('Tutor/Kotak_Saran') ?>" >Read More >> </a></h5>
							</div>
						</div>
						<div class="clearfix"></div>
					</div>
